...
import {
  getStepTypeLabel,
  createStepTemplate,
  validateAndFixReplayTargets,
} from "@/lib/utils/stepHelpers";
import {
  getAvailableVariables,
  validateFormConditionalLogic,
  validateMessageVariables,
} from "@/lib/utils/formValidation";
import { useFormBuilderTour } from "@/lib/hooks/useFormBuilderTour";
...
export default function FormEditorPage() {
        ...
        steps: form.steps.map(s => {
          if (s.id === selectedStepId && s.conditionalLogic) {
            return {
              ...
              conditionalLogic: {
                ...s.conditionalLogic,
                showIf: [...s.conditionalLogic.showIf, { variableName, operator: operator as ConditionalOperator, value }]
              }
            };
          }
          return s;
  ...
  const handleSave = async () => {
    ...
    if (!validation.isValid) {
      setError(
        "Please check conditional logic in all steps. Some steps reference variables that haven't been collected yet.",
      );
      setIsSaving(false);
    ...
    const { steps: fixedSteps, errors: replayErrors } =
      validateAndFixReplayTargets(form.steps);
  ...
  const handleRename = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSuccessMessage(
          "Form renamed successfully! The public URL has been updated.",
        );
  ...
  const selectedStepIndex =
    form?.steps.findIndex((s) => s.id === selectedStepId) ?? -1;
  const availableVariables =
    form && selectedStepIndex >= 0
    ...
    <div className="flex h-[calc(100vh-6rem)] flex-col">
      ...
      <div className="flex flex-1 gap-4">
                    ...
                    <div className="relative">
                      ...
                      {showAddStepMenu && (
                        <>
                          ...
                          <div className="absolute top-0 left-full z-20 ml-2 w-64 rounded-lg border bg-white shadow-xl dark:border-gray-700 dark:bg-gray-800">
                            <div className="border-b px-4 py-3 text-sm font-semibold text-gray-700 dark:border-gray-700 dark:text-gray-300">
                              Choose step type
                            </div>
                            <div className="py-1">
                              {Object.values(StepType).map((type) => (
                                <button
                                  ...
                                  className="flex w-full items-center px-4 py-3 text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700"
                                >
        ...
        <div className="flex-1">
          <Card data-tour="step-editor" className="h-full overflow-y-auto">
            {selectedStep ? (
              <StepEditor
                key={selectedStep.id}
                step={selectedStep}
                        ...
                        <div>
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                              If set, this name will be shown to users instead
                              of &quot;{form.name}&quot;
                            </p>
                          </div>
                        </div>
          ...
          <div className="space-y-4">
            <div className="rounded-lg bg-yellow-50 p-4 dark:bg-yellow-900/20">
              <p className="text-sm text-yellow-800 dark:text-yellow-200">
                ⚠️ <strong>Warning:</strong> Changing the form name will change
                the public URL and break any existing links that have been
                shared.
              </p>
              {form?.status === "published" && (
                <p className="mt-2 text-sm text-yellow-800 dark:text-yellow-200">
                  This form is currently <strong>published</strong>. Any shared
                  links will stop working after renaming.
                </p>
              )}
            </div>
